 ...
 *
 * Everything below the hero is rendered in the order the editor chose on the
 * page's own tab - which is why the body is a loop over section keys rather
 * than one section written after another. The hero is not in that list: it is
 * the page's opening and moving it below anything else would only ever be a
 * mistake. A section still renders nothing when its own heading is empty, so
 * the order is about arrangement, never about switching a section on.
 *
 * @package Antradus
 */
...
$antradus_order = antradus_section_order(
	$antradus_p . 'order',
	array_keys( antradus_audience_sections_list() )
);
?>
...
<?php foreach ( $antradus_order as $antradus_key ) : ?>
	<?php
	switch ( $antradus_key ) :
		...
		case 'content':
			/*
			 * Whatever the page itself holds - a screenshot, a gallery, three
			 * paragraphs written for this audience - renders here, in the
			 * design, so the editor is still worth opening. An empty page
			 * prints nothing at all.
			 */
			if ( '' === trim( wp_strip_all_tags( $antradus_content ) ) && false === strpos( $antradus_content, '<img' ) ) {
				break;
			}
			?>
		...
		case 'groups':
			if ( ! $antradus_groups || ! $antradus_f( 'groups_title' ) ) {
				break;
			}
			?>
		...
		case 'trends':
			// Renders nothing unless this page's own `*_trends_*` keys are set.
			get_template_part( 'template-parts/section', 'trends', array( 'prefix' => $antradus_p ) );
			break;

		case 'flow':
			if ( ! $antradus_flow || ! $antradus_f( 'flow_title' ) ) {
				break;
			}
			?>
			<section class="ant-section">
				<div class="ant-wrap">
					...
					<?php
					/*
					 * Numbered by position, not by hand. The steps on the
					 * contact page carry their own "01", which is right there -
					 * three steps that never change. Here the editor can add or
					 * reorder a step, and a typed number would be wrong the
					 * moment they did.
					 */
					?>
		...
		case 'compat':
			// Same again: empty `*_compat_*` keys mean no diagram on this page.
			get_template_part( 'template-parts/section', 'compat', array( 'prefix' => $antradus_p ) );
			break;

		case 'plan':
			if ( ! $antradus_plans || ! $antradus_f( 'plan_title' ) ) {
				break;
			}
			?>
		...
		case 'switch':
			/*
			 * The way back out. A page that asks somebody to identify
			 * themselves has to let the ones who guessed wrong leave without
			 * using the back button, so the other audience page is always one
			 * click away from this one.
			 */
			if ( ! $antradus_f( 'switch_title' ) || '' === $antradus_switch ) {
				break;
			}
			?>
			<section class="ant-section">
